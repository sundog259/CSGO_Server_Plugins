...
public Plugin myinfo = 
{
	...
	author = "Powerlord, Zuko, and AlliedModders LLC, Anubis edition",
...
public void OnPluginStart()
{
	...
	g_Cvar_SoundSet = CreateConVar("mce_sounds_soundset", "hl1", "Sound set to use, optimized for hl1 by default.  Sound sets are defined in addons/sourcemod/configs/mapchooser_extended_sounds.cfg.  Takes effect immediately if sm_mapvote_downloadallsounds is 1, otherwise at map change.", FCVAR_NONE);
...
void SetSoundSetFromCVar()
{
	...
	if (FindStringInArray(g_SetNames, soundSet) == -1 && !StrEqual(soundSet, "hl1", true))
	{
					...
					do
					{
						...
						if (KvGotoFirstSubKey(soundsKV)) {
							...
							do
							{
								...
								int typeKey = FindStringInArray(g_TypeNames, currentType);
...
SoundType RetrieveSound(Handle soundsKV, bool isBuiltin, char[] soundFile, int soundFileSize)
{
	if (isBuiltin)
	{
...
void BuildDownloadsTable(Handle currentSoundSet)
{
