...
public void OnPluginStart()
{
	if (GetEngineVersion() != Engine_CSGO) {
		SetFailState("[System Init] Engine error: This plugin only works on Counter-Strike: Global Offensive.");
	}
	...
	if ((hSDKCallDestructorParticleDictionary = EndPrepSDKCall()) == null) {
		SetFailState("[GameData Validation] Failed to load SDK call \"CParticleSystemDictionary::~CParticleSystemDictionary\". Update signature in \"%s\"", PLUGIN_CONFIG);
	}
	...
	if ((hSDKCallTableDeleteAllStrings = EndPrepSDKCall()) == null) {
		SetFailState("[GameData Validation] Failed to load SDK call \"CNetworkStringTable::DeleteAllStrings\". Update signature in \"%s\"", PLUGIN_CONFIG);
	}

	/*_________________________________________________________________________________________________________________________________________*/

	// Load other offsets
	fnInitGameConfAddress(gServerData.Config, pParticleSystemDictionary, "CParticleSystemMgr::m_pParticleSystemDictionary");
	...
	LogMessage("[System Init] Loaded \"Particle System Fix\"");
}

public void OnMapInit()
{
	// Load internal particles
	ParticlesOnLoad();
}
...
void ParticlesOnLoad()
{
	...
	if (pTable != Address_Null && !iCount) { /// Validate that table is exist and it empty
		...
		if (hFile == null) {
			LogMessage("[Config Validation] Error opening file: \"particles/particles_manifest.txt\"");
			return;
		}
		...
		while (hFile.ReadLine(sBuffer, sizeof(sBuffer))) {
			...
			if (iQuotes == 4) {
				...
				// Precache default particles
				if (sBuffer[0] == '!') {
					PrecacheGeneric(sBuffer[1], true);
...
void ParseParticleManifestKv(KeyValues hKv, bool perMap = false, bool alwaysPreload = false)
{
	...
	if (hKv.GotoFirstSubKey(false))
		do {
			...
			if (FileExists(sPCF, true)) {
				...
				PrecacheGeneric(sPCF, alwaysPreload || preload);
			}
...
stock Address CNetworkStringTableContainer_FindTable(const char[] tableName)
{
	...
	if (hFindTable == INVALID_HANDLE) {
		...
		if (!PrepSDKCall_SetFromConf(gServerData.Config, SDKConf_Virtual, "CNetworkStringTableContainer::FindTable")) {
			LogError("[Find Table] Cant find the method CNetworkStringTableContainer::FindTable.");
			return Address_Null;
		}
		...
		if (hFindTable == INVALID_HANDLE) {
			LogError("[Find Table] Method CNetworkStringTableContainer::FindTable was not loaded right.");
			return Address_Null;
		}
	}
...
stock Address GetNetworkStringTableAddr()
{
	...
	if (pEngineServerStringTable == Address_Null) {
		...
		if (!gServerData.Config.GetKeyValue("VEngineServerStringTable", sInterfaceName, sizeof(sInterfaceName))) {
...
stock Address CreateEngineInterface(const char[] sInterfaceKey, Address ptr = Address_Null)
{
	...
	char sInterfaceName[64];
	if (!gServerData.Config.GetKeyValue(sInterfaceKey, sInterfaceName, sizeof(sInterfaceName))) {
 ...
 **/
stock void fnInitGameConfOffset(GameData gamedata, int &iOffset, char[] sKey)
{
	// Validate offset
	if ((iOffset = gamedata.GetOffset(sKey)) == -1) {
 ...
 **/
stock void fnInitGameConfAddress(GameData gamedata, Address &pAddress, char[] sKey)
{
	// Validate address
	if ((pAddress = gamedata.GetAddress(sKey)) == Address_Null) {
		SetFailState("[GameData Validation] Failed to get address: \"%s\"", sKey);
	}
}
