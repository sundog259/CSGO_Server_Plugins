...
public void OnPluginStart() {
	...
	RegAdminCmd("sm_god", Command_God, ADMFLAG_GENERIC, "sm_god [#userid|name] [0/1] - Toggles godmode on player(s)");
	RegAdminCmd("sm_buddha", Command_Buddha, ADMFLAG_GENERIC, "sm_buddha [#userid|name] [0/1] - Toggles buddha mode on player(s)");
	RegAdminCmd("sm_mortal", Command_Mortal, ADMFLAG_GENERIC, "sm_mortal [#userid|name] - Makes specified players mortal");
...
public Action Command_God(int client, int args) {
	...
	if (args != 0 && !CheckCommandAccess(client, "sm_godmode_admin", ADMFLAG_SLAY, true)) {
		...
		CPrintToChat(client, "[SM] Usage: sm_god");
	...
	if (args > 2) {
		...
		CPrintToChat(client, "[SM] Usage: sm_god [#userid|name] [0/1]");
		return Plugin_Handled;
	...
	if (!args) {
		...
		if (g_iState[client] != 1) {
			...
			CPrintToChat(client, "[SM] God Mode {green} ON");
			g_iState[client] = 1;
		...
		// GodMode on
			...
			CPrintToChat(client, "[SM] God Mode {darkred} OFF");
			g_iState[client] = 0;
	...
	//Turn off godmode
		...
		CShowActivity2(client, "[SM] ", "{darkred}Disabled {default}God Mode on {green}%s", target_name);
		for (int i = 0; i < target_count; i++) {
			
			if (bAnnounce)
				CPrintToChat(target_list[i], "[SM] An admin has {darkred}removed {default}your God Mode!");
		...
		for (int i = 0; i < target_count; i++) {
			...
			//Turn off godmode
				...
				CShowActivity2(client, "[SM] ", "{darkred}Removed {default}God Mode from {green}%s", target_name);
...
public Action Command_Buddha(int client, int args) {
	...
	if (args != 0 && !CheckCommandAccess(client, "sm_buddhamode_admin", ADMFLAG_SLAY, true)) {
		...
		CPrintToChat(client, "[SM] Usage: sm_buddha");
		return Plugin_Handled;
	...
	if (args > 2) {
		...
		CPrintToChat(client, "[SM] Usage: sm_buddha [#userid|name] [0/1]");
		return Plugin_Handled;
	...
	if (!args) {
		...
		if (g_iState[client] != 2) {
			...
			CPrintToChat(client, "[SM] Buddha Mode {green}ON");
			g_iState[client] = 2;
		...
		// GodMode on
			...
			SetEntProp(client, Prop_Data, "m_takedamage", 2, 1);
			CPrintToChat(client, "[SM] Buddha Mode {darkred}OFF");
			g_iState[client] = 0;
	...
	//Turn off Buddha
		...
		CShowActivity2(client, "[SM] ", "{darkred}Disabled {default}Buddha Mode on {green}%s", target_name);
		
		for (int i = 0; i < target_count; i++) {
			
			if (bAnnounce)
				CPrintToChat(target_list[i], "[SM] An admin has {darkred}removed {default}your Buddha Mode!");
		...
		for (int i = 0; i < target_count; i++) {
			...
			//Turn off godmode
				...
				CShowActivity2(client, "[SM] ", "{darkred}Removed {default}Buddha Mode from {green}%s", target_name);
...
public Action Command_Mortal(int client, int args) {
	...
	if (args != 0 && !CheckCommandAccess(client, "sm_mortalmode_admin", ADMFLAG_SLAY, true)) {
		...
		CPrintToChat(client, "[SM] Usage: sm_mortal");
		return Plugin_Handled;
	...
	if (args < 0 || args > 1) {
		...
		CPrintToChat(client, "[SM] Usage: sm_mortal [#userid|name]");
		return Plugin_Handled;
	...
	if (!args) {
		...
		SetEntProp(client, Prop_Data, "m_takedamage", 2, 1);
		CPrintToChat(client, "[SM] You are now {darkred}mortal{default}!");
		g_iState[client] = 0;
		return Plugin_Handled;
	...
	CShowActivity2(client, "[SM] ", "{default}Made {green}%s {darkred}mortal{default}!", target_name);
	...
	for (int i = 0; i < target_count; i++) {
		...
		if (g_iState[target_list[i]] != 0) {
			...
			if (chat)
				CPrintToChat(target_list[i], "[SM] An admin has made you {darkred}mortal{default}!");
